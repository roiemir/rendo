...
Rendo.prototype.renderContents = function (contents, model) {
    ...
    while (o >= s) {
        ...
        var component = null;
        var fromModel = false;
        var embed = contents.slice(o, c);
        var i = search(embed, function (x) { return !isWhitespace(x); });
        embed = embed.slice(i);
        if (embed[0] == ':' || embed[0] == '+') {
            fromModel = embed[0] == '+';
            i = search(embed, isWhitespace);
            component = embed.slice(1, i);
            embed = embed.slice(i + 1);
        }

        var expressions = oil.parse(embed);
