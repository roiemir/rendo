            ...
            for (var m = 0; m < models.length; m++) {
                if (component) {
                    if (fromModel) {
                        ...
                        if (renderContents) {
                            result += this.renderContents(renderContents, new Model(model, models[m], [{model: model, parameter: models[m]}].concat(parameters)));
                        }
                    }
                    else {
                        result += this.render(component, new Model(model, models[m]), [{model: model, parameter: models[m]}].concat(parameters));
                    }
                }
                else {
