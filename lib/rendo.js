...
var binary = {
    "==": function (left, right) {
        if (left && right && left.valueOf instanceof Function && right.valueOf instanceof Function) {
...
var modifiers = {
    ...
    sum: function (value) {
        if (value == null) {
            ...
            return value.reduce(function (c, p) { return c + p; });
        }
        return null;
    },
    ...
    padStart: function (value, length, padString) {
        return (value == null ? "" : value.toString()).padStart(length, padString);
    },
    padEnd: function (value, length, padString) {
        return (value == null ? "" : value.toString()).padEnd(length, padString);
    },
...
function evaluate(expression, model) {
    ...
    if (expression instanceof Date) {
        expression = toDateObject(expression);
    }
    return expression;
}
...
Rendo.prototype.renderContents = function (contents, model) {
    ...
    while (o >= s) {
        ...
        if (expressions.length > 0) {
            ...
            else if (models.constructor !== Array) {
                models = [models];
            }
            else {
                isArray = true;
